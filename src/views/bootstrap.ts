...
export function renderBootstrapTab(
  ...
  controls.innerHTML = `
    <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
      <div style="display:flex;align-items:center;gap:6px">
        ...
        <select id="boot-iter" style="font-size:12px">
      ...
      <div style="display:flex;align-items:center;gap:6px">
        ...
        <select id="boot-level" style="font-size:12px">
      ...
      <div style="display:flex;align-items:center;gap:6px">
        ...
        <select id="boot-method" style="font-size:12px">
          <option value="stability" selected>Stability</option>
          <option value="threshold">Threshold</option>
        </select>
      </div>
      <button id="run-bootstrap" class="btn-primary" style="font-size:12px;padding:6px 16px">Run Bootstrap</button>
    </div>
  `;
  grid.appendChild(controls);

  const resultsDiv = document.createElement('div');
  resultsDiv.id = 'boot-results';
  ...
  setTimeout(() => {
    document.getElementById('run-bootstrap')?.addEventListener('click', () => {
      ...
      const resultsEl = document.getElementById('boot-results')!;
      ...
      setTimeout(() => {
        try {
          ...
          renderBootstrapResults(resultsEl, result, networkSettings);
        } catch (err) {
...
function renderBootstrapResults(
  ...
  netPanel.innerHTML = `
    ...
    <div id="viz-boot-network" style="width:100%;height:${h}px"></div>
  `;
